  ...
  return <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh',padding:20,background:T.bg}}>
    <div style={{background:T.panel,border:`1px solid ${T.brd2}`,padding:'40px 32px',width:'100%',maxWidth:380,borderRadius:16,boxShadow:`0 24px 80px ${T.sh}`}}>
      <div style={{textAlign:'center',marginBottom:32}}>
        ...
        <div style={{fontFamily:'JetBrains Mono,monospace',fontSize:10,color:T.tm,letterSpacing:2}}>v9.16 - MULTI-TF · PARALLEL · 150+ PAIRS</div>
      </div>
...
function SettingsTab({T,dark,setDark,cfg,onSave,user,onLogout,onLogin,saving,saved,pushOn,setPushOn}){
  ...
  return <div>
    ...
    <Sec title="RSI Cap" col={T.acc}>
      <Row label="RSI Cap" sub={loc.rsiCapEnabled?`Bull 50->${loc.rsiTolerance>=50?'':50+loc.rsiTolerance} | Bear ${loc.rsiTolerance>=50?'0':50-loc.rsiTolerance}->50`:'Any RSI above/below 50'}><Tog on={loc.rsiCapEnabled} onChange={v=>set('rsiCapEnabled',v)} T={T} color={T.acc}/></Row>
      {loc.rsiCapEnabled&&<Pills opts={[[10,'10'],[15,'15'],[20,'20'],[25,'25'],[30,'30'],[50,'50']]} val={loc.rsiTolerance} pick={v=>set('rsiTolerance',v)} c={T.acc}/>}
    </Sec>
...
const INIT_CFG=()=>{try{const s=JSON.parse(localStorage.getItem('ema_v9_cfg')||'null');return s?{...DEFAULT_SETTINGS,...s,slope:{...DEFAULT_SETTINGS.slope,...(s.slope||{})},timeframe:s.timeframe||'15m'}:{...DEFAULT_SETTINGS,timeframe:'15m'}}catch{return{...DEFAULT_SETTINGS,timeframe:'15m'}}}
...
export default function App(){
    ...
    for(let gi=0;gi<pairs.length;gi+=CHUNK){
      ...
      ws.onmessage=(ev)=>{
        try{
          ...
          const k=msg?.data?.k
          if(!k||!k.x)return // only process confirmed CLOSED klines
          const sym=k.s
          const pair=grp.find(p=>p.s===sym)
          if(!pair)return
          // Update candle cache with the new closed candle
          const newCd={o:+k.o,h:+k.h,l:+k.l,c:+k.c,v:+k.v,t:k.t}
          const cached=candleCacheRef.current[sym]||[]
          const updated=[...cached.slice(-99),newCd]
  ...
  const runScan=useCallback(async(loopNum=1)=>{
    ...
    // ── Auto mode: switch to WebSocket instead of timer loop ─
    // WebSocket listens for each 15m kline close and runs same
    // EMA detection on that pair — no need to re-fetch all pairs
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 18px',background:T.panel,borderBottom:`1px solid ${T.brd}`,position:'relative',overflow:'hidden',flexShrink:0}}>
        ...
        <div style={{fontFamily:'Rajdhani,sans-serif',lineHeight:1}}>
          <div style={{fontSize:20,fontWeight:900,letterSpacing:2}}>
            ...
            <span style={{marginLeft:8,fontSize:10,color:T.bull,verticalAlign:'middle',border:`1px solid ${T.bull}44`,padding:'2px 7px',background:`${T.bull}0c`,fontFamily:'JetBrains Mono,monospace',borderRadius:4,letterSpacing:1}}>v9.16</span>
          </div>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
